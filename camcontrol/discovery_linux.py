...
def find_devices() -> List[DeviceInfo]:
    ...
    Typically returns /dev/ttyUSB* (USB-serial) and /dev/ttyACM* (CDC ACM).
    """
    ...
    for p in ports:
        ...
        # Prefer typical Linux serial device nodes; still include others.
        devices.append(
            DeviceInfo(
                device=device,
...
def pick_default_device(devices: List[DeviceInfo]) -> Optional[DeviceInfo]:
    ...
    if len(ch340) == 1:
        return ch340[0]
    if len(ch340) > 1:
        # Prefer ttyUSB before ttyACM, then lexical.
